...
export type OidcProof = components["schemas"]["OidcProof"];
...
export class CubeSigner {
   ...
   */
  async oidcProve(oidcToken: string, orgId: string): Promise<OidcProof> {
   ...
   */
  async oidcVerify(orgId: string, oidcProof: OidcProof) {
    const resp = await (
      await this.management()
    ).post("/v0/org/{org_id}/oidc/verify", {
      params: { path: { org_id: orgId } },
      body: oidcProof,
      parseAs: "json",
    });
