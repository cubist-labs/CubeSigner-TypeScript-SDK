...
const RPC_PROVIDER: string = env("RPC_PROVIDER", "https://rpc.ankr.com/eth_sepolia")!;
...
describe("sendTransaction", () => {
  ...
  beforeAll(async () => {
    ...
    signer = new Signer(GENERATED_ADDRESS, client, { provider });
...
describe("sendMfaTransaction", () => {
  ...
  beforeAll(async () => {
    client = await cs.CubeSignerClient.create(CUBE_SIGNER_MANAGEMENT_TOKEN);
    ...
    provider = new ethers.providers.JsonRpcProvider(RPC_PROVIDER);
    signer = new Signer(GENERATED_ADDRESS, roleClient, { provider });
