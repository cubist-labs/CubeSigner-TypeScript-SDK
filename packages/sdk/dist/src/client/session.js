...
import { apiFetch, assertOk } from "../fetch.js";
 ...
 */
export function parseSessionLike(session) {
 ...
 * ```
 */
export class NoRefreshSessionManager {
    /**
 ...
 */
export class ExclusiveSessionManager {
    constructor() {
...
export function refresh(session) {
    ...
    return retryOn5XX(() => apiFetch("/v1/org/{org_id}/token/refresh", "patch", {
 ...
 */
export function metadata(session) {
 ...
 */
const isWithinBuffer = (timeInSeconds) => timeInSeconds < Date.now() / 1000 + DEFAULT_EXPIRATION_BUFFER_SECS;
export const isStale = (session) => isWithinBuffer(session.session_info.auth_token_exp);
export const isRefreshable = (session) => !isWithinBuffer(session.session_exp ?? Number.POSITIVE_INFINITY) &&
 ...
 * @returns session.
 */
export function parseBase64SessionData(sessionDataString) {
 ...
 */
export function serializeRefreshToken(info) {
