import type { ApiClient, ContractAddress, EvmTxCmp, SolanaTxCmp, KeyWithPoliciesInfo, PageOpts, RoleInfo, Scope, SessionData, SessionLifetime, BabylonStakingRequest, OperationKind, MfaReceipts, CubeSignerResponse, Empty, RestrictedActionsMap, GetRoleKeyOptions, EditPolicy, MfaPolicy, RoleInfoJwt } from "./index.ts";
import { Key, SignerSessionInfo } from "./index.ts";
import type { RoleAttestationQuery } from "./schema_types.ts";
type NameOrAddressOrNull = string | null;
/** Only allow the following operations */
export type OperationAllowlist = {
    OperationAllowlist: OperationKind[];
};
/**
...
export type TxReceiver = {
    TxReceiver: NameOrAddressOrNull | NameOrAddressOrNull[];
};
/**
...
export declare const AllowDiffieHellmanExchange: "AllowDiffieHellmanExchange";
...
export type AllowPolicy = AllowRawBlobSigning | AllowDiffieHellmanExchange | AllowEip191Signing | AllowEip712Signing | AllowBtcMessageSigning;
...
/**
 * A reference to an org-level named policy using its id.
 * We cannot use the policy's name in this format.
 */
export type NamedPolicyReference = {
    Reference: PolicyReference;
};
/** Explicit "permit" vs "deny" policy outcome, with or without a descriptive message. */
export type Const = ConstOutcome | {
    outcome: ConstOutcome;
    message: string;
};
...
export type KeyDenyPolicy = Const | Cel | OperationAllowlist | TxReceiver | TxDeposit | TxValueLimit | TxGasCostLimit | IfErc20Tx | AssertErc20Tx | AssertTransferOnlyTx | AssertContractTx | SuiTxReceivers | BtcTxReceivers | SourceIpAllowlist | TimeLock | SolanaInstructionPolicy | BtcSegwitValueLimit | RequireMfa | RequireRoleSession | BabylonStaking | WebhookPolicy | PolicyAnd | PolicyOr | PolicyNot | PolicyIte | NamedPolicyReference;
/**
 * Key policy
 *
 * @example [
...
export type KeyPolicyRule = KeyDenyPolicy | AllowPolicy | PolicyReference;
/** Role policy */
...
export type Conditionals = Conditional | {
...
export declare class Role {
     ...
     */
    attest(query?: RoleAttestationQuery): Promise<RoleInfoJwt>;
    /**
     * Delete the role.
     *
     ...
     * @throws If MFA is required and no MFA receipts are provided
     */
    setPolicy(policy: RolePolicy, mfaReceipt?: MfaReceipts): Promise<void>;
    /**
     ...
     * @throws If MFA is required and no MFA receipts are provided
     */
    appendPolicy(policy: RolePolicy, mfaReceipt?: MfaReceipts): Promise<void>;
    /**
     ...
     * @throws If MFA is required and no MFA receipts are provided
     */
    setRestrictedActions(restrictedActions: RestrictedActionsMap, mfaReceipt?: MfaReceipts): Promise<void>;
    /**
     ...
     */
    addUser(userId: string, mfaReceipt?: MfaReceipts): Promise<CubeSignerResponse<{
        status: string;
    }>>;
    /**
     ...
     * @returns an empty response, or a response that can be used to approve MFA if needed.
     */
    removeUser(userId: string, mfaReceipt?: MfaReceipts): Promise<CubeSignerResponse<{
        status: string;
    }>>;
    /**
     ...
     */
    getKey(keyId: string, opts?: GetRoleKeyOptions): Promise<KeyWithPolicies>;
    /**
     ...
     */
    addKeys(keys: Key[], policy?: KeyPolicy): Promise<CubeSignerResponse<Empty>>;
    /**
     ...
     * @returns A CubeSigner response indicating success or failure.
     */
    addKey(key: Key, policy?: KeyPolicy): Promise<CubeSignerResponse<Empty>>;
    /**
     ...
     * @returns A CubeSigner response indicating success or failure.
     */
    removeKey(key: Key): Promise<CubeSignerResponse<Empty>>;
    /**
