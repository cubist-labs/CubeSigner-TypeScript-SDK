 ...
 */
function envEquals(left: EnvInterface, right: EnvInterface): boolean {
  return (
    left.SignerApiRoot === right.SignerApiRoot &&
 ...
 */
function validatePreferredEnv(
  preferredEnv: EnvInterface | undefined,
  sessionMeta: SessionMetadata,
): EnvInterface | undefined {
  ...
  if (!Object.values(sessionMeta.env).some((env) => envEquals(env, preferredEnv))) {
    throw new Error(
      `The current session does not allow the '${preferredEnv.SignerApiRoot}' environment`,
    );
  }
...
export class BaseClient extends EventEmitter<ClientEvents> {
  ...
  constructor(
    ...
    this.#preferredEnv = validatePreferredEnv(config?.preferredEnv, sessionMeta);
