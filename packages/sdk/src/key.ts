...
import type { ApiClient, CubeSignerResponse, ErrResponse, JsonValue, MfaReceipt } from ".";
...
export class Key {
   ...
   */
  async signAva(tx: AvaTx, mfaReceipt?: MfaReceipt): Promise<CubeSignerResponse<AvaSignResponse>> {
  ...
  private async update(request: UpdateKeyRequest): Promise<KeyInfo> {
    this.#data = await this.#apiClient.keyUpdate(this.id, request);
    return this.#data;
  }
  ...
  private async fetch(): Promise<KeyInfo> {
    this.#data = await this.#apiClient.keyGet(this.id);
    return this.#data;
  }
}
