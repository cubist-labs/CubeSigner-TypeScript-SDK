...
describe("import", () => {
  ...
  it("imports mnemonic and derives keys properly", async () => {
    ...
    const expectedAddr = "bc1p438wagdrgvmccdf7a5tr6l5w82ae5az7dz3ve49qc2fauvzxg2wscr2lhd";
    // `derive_key` only returns *newly*-derived keys, so it yields `undefined` when a key
    // at this path already exists (e.g. left over from a prior interrupted run). Fall back
    // to looking it up so the test is robust to pre-existing state.
    let deriveResponse: cs.Key | undefined = await org.deriveKey(
      cs.Secp256k1.Taproot,
      "m/86'/0'/4'/0/0",
      mnemonic.materialId,
    );
    deriveResponse ??= await org.getKeyByMaterialId(cs.Secp256k1.Taproot, expectedAddr);
    expect(deriveResponse).toBeDefined();
    toDelete.push(deriveResponse);
    expect(deriveResponse.materialId).toEqual(expectedAddr);
