...
describe("AzureKeyVaultSessionManager", () => {
  ...
  beforeAll(async () => {
    ...
    mockGetSecret = vi.fn().mockImplementation((name: string) => {
    ...
    mockSetSecret = vi.fn().mockImplementation((name: string, value: string) => {
      secrets[name] = value;
      return Promise.resolve({
        value,
        name,
    ...
    vi.mocked(SecretClient).mockImplementation(function () {
      return {
  ...
  it("Error handling for missing secret", async () => {
    const sessionMgr = new csAzKv.AzureKeyVaultSessionManager(vaultUrl, "non-existent-secret");
  ...
  it("Manager can update secret that session manager reads", async () => {
    ...
    const freshSession = await client
      .org()
      .createSession("coordinated-test", ["manage:org:user:get"], {
        grace: 0,
      });
...
describe("AzureExclusiveKeyVaultSessionManager", () => {
  ...
  it("Works with CubeSigner client", async () => {
    ...
    const user = await client.user();
    const orgIds = user.orgs.map((o) => o.org_id);
    expect(orgIds).toContain(client.orgId);
  });
  ...
  it("Handles token refresh properly", async () => {
    ...
    const freshSession = await client
      .org()
      .createSession("exclusive-refresh-test", ["manage:org:user:get"], { grace: 0 });
