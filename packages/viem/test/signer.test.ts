...
describe("Basic Viem signing: signMessage/TypedData", () => {
  let createdKeys: Key[];
  ...
  beforeAll(async () => {
    ...
    expect(aboutMe.orgs.length).toEqual(1);
    ...
    await role.addKey(secp, [AllowEip712Signing, AllowEip191Signing]);
  ...
  it("signMessage(string)", async () => {
    ...
    expect(verifyMessage({ address, message, signature })).resolves.toBe(true);
  });
