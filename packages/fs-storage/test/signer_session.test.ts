...
describe("SignerSessionManager", () => {
  ...
  it("expired auth token triggers onSessionExpired", async () => {
    ...
    const err = await assertForbidden(ssNoRefresh.keys());
    expect(err).to.be.instanceof(SessionExpiredError);
    expect(err.operation).to.be.equal("listTokenKeys");
  ...
  it("expired session triggers onSessionExpired", async () => {
    ...
    {
      console.log("Using expired token returns FORBIDDEN");
      const err = await assertForbidden(ss.keys());
      expect(sessionExpired).to.equal(1);
      expect(err).to.be.instanceof(SessionExpiredError);
      expect(err.operation).to.be.equal("listTokenKeys");
    }
    ...
    {
      console.log("Refreshing expired token returns FORBIDDEN");
      const err = await assertForbidden(ss.sessionMgr.refresh());
      expect(sessionExpired).to.equal(2);
      expect(err).to.be.instanceof(SessionExpiredError);
      expect(err.operation).to.be.equal("signerSessionRefresh");
    }
