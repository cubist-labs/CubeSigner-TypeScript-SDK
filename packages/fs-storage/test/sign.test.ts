...
import { assert4xx, assertForbidden, assertPreconditionFailed, deleteKeys } from "./helpers";
...
describe("Sign", () => {
  ...
  it("evm", async () => {
    ...
    // request then reject using CubeSigner management session
    {
      ...
      await resp.reject(org);
      const err = await assert4xx(session.mfaGet(mfaId));
    ...
    // request then approve using TOTP code
    {
      ...
      userMfaSession.sessionMgr.events.onUserMfaFailed(handler);
      const err = await assertForbidden(resp.approveTotp(userMfaSession, "AAAAAA"));
      expect(err.errorCode).to.eq(<CsErrCode>"MfaTotpBadCode");
      expect(mfaFailed).to.eq(1);
      userMfaSession.sessionMgr.events.unregisterOnError(handler);
    ...
    // request then reject using TOTP code
    {
      ...
      const mfaId = resp.mfaId();
      expect(mfaId).to.exist;
      await resp.rejectTotp(userMfaSession, code);
      const err = await assert4xx(session.mfaGet(mfaId));
      expect(err.errorCode).to.eq(<CsErrCode>"MfaRequestNotFound");
      console.log(`Rejected ${mfaId} using CubeSigner`);
    }
  ...
  it("eip191", async () => {
    ...
    const err = await assertPreconditionFailed(session.signEip191(secp, req));
  ...
  it("eip712", async () => {
    ...
    const err = await assertPreconditionFailed(session.signEip712(secp, req));
  ...
  it("blob", async () => {
    ...
    await Promise.all(
      [
        ...
        const err = await assertPreconditionFailed(session.signBlob(key, req));
