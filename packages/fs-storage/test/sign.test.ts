...
describe("Sign", () => {
   ...
   */
  async function assertDryRunResp<U>(resp: CubeSignerResponse<U>): Promise<SignDryRun> {
    const signDryRunResp = resp.asSignDryRun();
    assert(signDryRunResp !== undefined);
    await Promise.all(
      signDryRunResp.mfa_requests.map((mfa) =>
        assertErrorCode("MfaRequestNotFound", org.getMfaRequest(mfa.id).fetch()),
      ),
    );
    return signDryRunResp;
  }
  ...
  it("evm", async () => {
    ...
    const noDryRunReq: EvmSignRequest = {
      chain_id: 1,
      tx: {
        type: "0x00",
        gas: "0x61a80",
    ...
    const dryRunReq: EvmSignRequest = {
      ...noDryRunReq,
      dry_run: true,
    };

    for (const req of [dryRunReq, noDryRunReq]) {
      const resp = await signingKey.signEvm(req);
      await delay(1000);
      const hist = await secp.history({ size: 1, all: false });
      if (req === dryRunReq) {
        const dryRunResp = await assertDryRunResp(resp);
        expect(dryRunResp.mfa_requests).to.deep.equal([]);
        expect(hist.length).to.eq(0); // relies on running 'dryRunReq' first
      } else {
        expect(resp.isSuccess()).to.be.true;
        const data = resp.data();
        console.log(`Signed EVM: ${JSON.stringify(data)}`);
        expect(hist.length).to.eq(1);
        expect(hist[0].result).to.deep.eq(data);
    ...
    for (const req of [noDryRunReq, dryRunReq]) {
      const resp = await signingKey.signEvm(req);
      if (req === dryRunReq) {
        const dryRunResp = await assertDryRunResp(resp);
        expect(dryRunResp.mfa_requests.length).to.eq(1);
      } else {
        expect(resp.requiresMfa()).to.be.true;
        const sig = await resp.approve(client);
        console.log(
          `Signed EVM after MFA approved using CubeSigner: ${JSON.stringify(sig.data())}`,
        );
      }
    }

    // request then reject using CubeSigner management session
    for (const req of [noDryRunReq]) {
      const resp = await signingKey.signEvm(req);
    ...
    for (const req of [noDryRunReq]) {
      ...
      const mfaId = resp.mfaId();
      expect(mfaId).to.exist;
      const mfa = await org.getMfaRequest(mfaId!).approve();
      const receipt = await mfa.receipt();
      const sig = await signingKey.signEvm(req, receipt);
      assert(!sig.requiresMfa());
      console.log(`Signed EVM after MFA approved: ${JSON.stringify(sig.data())}`);
    }

    // do the same, but using the user session assuming the role to submit the approved request
    for (const req of [noDryRunReq]) {
      const resp = await signingKey.signEvm(req);
      assert(resp.requiresMfa());
      const mfaId = resp.mfaId();
      ...
      // create a user session without any 'sign' scopes (which would not be allowed to sign)
      const newSession = await org.createSession("no sign scopes", ["manage:session:create"]);
      const newClient = await CubeSignerClient.create(newSession);

      // with that session, assume the role session, which is allowed to sign with the 'secp' key
      const sig = await newClient.apiClient
        .assumeRole(role.id)
        // unnecessarily calling 'withTargetOrg' with the same org id just
        // to ensure that it preserves the existing client configuration
        .withTargetOrg(org.id)
        .signEvm(secp, req, receipt);
      assert(!sig.requiresMfa());
      console.log(`Signed EVM after MFA approved assuming role: ${JSON.stringify(sig.data())}`);
    }
  ...
  it("eip191", async () => {
    ...
    const noDryRunReq: Eip191SignRequest = {
      data: encodeToHex(message),
      metadata: `EIP-191 message: ${message}`,
    };
    const dryRunReq: Eip191SignRequest = { ...noDryRunReq, dry_run: true };
    ...
    for (const req of [noDryRunReq, dryRunReq]) {
      ...
      if (req === dryRunReq) {
        ...
        expect(resp.isSuccess()).to.be.true;
        console.log(`Signed EIP-191 message: ${JSON.stringify(resp.data())}`);
      }
    }
    ...
    for (const req of [dryRunReq, noDryRunReq]) {
      ...
      if (req === dryRunReq) {
        ...
        expect(resp.asMfaRequired()).to.not.be.undefined;
        const sig = await resp.approve(client);
        console.log(
  ...
  it("eip712", async () => {
    ...
    const noDryRunReq: Eip712SignRequest = {
      chain_id: 1,
      typed_data: <any>{
    ...
    const dryRunReq: Eip712SignRequest = {
      ...noDryRunReq,
      dry_run: true,
    };
    ...
    for (const req of [dryRunReq, noDryRunReq]) {
      ...
      if (req === dryRunReq) {
        ...
        expect(resp.isSuccess()).to.be.true;
        console.log(`Signed EIP-712 data: ${JSON.stringify(resp.data())}`);
      }
    }
  ...
  for (const dryRun of [true, false]) {
    it(`mfa ${dryRun ? "with" : "without"} dry run`, async () => {
      expect(await role.enabled()).to.equal(true);
      for (const keyType of [Ed25519.Aptos, Ed25519.Cardano]) {
        ...
        expect(resp.asAccepted()).to.not.be.undefined;
        if (dryRun) {
          expect(resp.requiresMfa()).to.equal(false);
          const dryRunResp = resp.asSignDryRun()!;
          expect(dryRunResp).to.not.be.undefined;
          expect(dryRunResp.policy_eval_tree).to.not.be.undefined;
          expect(dryRunResp.mfa_requests.length).to.equal(1);
          await assertErrorCode(
            "MfaRequestNotFound",
            org.getMfaRequest(dryRunResp.mfa_requests[0].id).fetch(),
          );
        } else {
          expect(resp.requiresMfa()).to.equal(true);
          const mr = resp.asMfaRequired()!;
          expect(mr).to.not.be.undefined;
          expect(mr.policy_eval_tree).to.not.be.undefined;
