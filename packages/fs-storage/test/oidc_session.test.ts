...
import { delay, deleteKeys, parseOidcToken } from "./helpers";
...
describe("OidcSessionManager", () => {
  ...
  it("target different regions", async () => {
    console.log("Exchanging token");
    const oidcToken = await loadCognitoOidcToken();
    const isInBeta = env.SignerApiRoot.endsWith("beta.signer.cubist.dev");
    const envs = isInBeta ? multiRegionEnvs.beta : MultiRegionEnv.create(env);
    const sessionData = await CubeSignerClient.createOidcSession(envs, orgId, oidcToken, [
      "manage:readonly",
    ]);
    const c = await CubeSignerClient.create(sessionData.data());
    expect(c.envs.length).toEqual(Object.values(envs.spec).length);
    const root = c.env.SignerApiRoot;

    for (const env of c.envs) {
      console.log("Using endpoint", env.SignerApiRoot);
      const client = c.withPreferredEnv(env);
      // the original client didn't change
      expect(c).not.toBe(client);
      expect(c.env.SignerApiRoot).toBe(root);
      // the new client uses the api root from the env
      expect(client.env.SignerApiRoot).toBe(env.SignerApiRoot);

      // and it also works (retry a few times because of cross-region consistency)
      for (let i = 0; i < 5; i++) {
        let user = undefined;
        try {
          user = await client.apiClient.userGet();
        } catch (e) {
          if ((e as ErrResponse).errorCode === "SessionNotFound") {
            console.log("SessionNotFound, waiting the retrying");
            await delay(1000);
            continue;
          } else {
            throw e;
        ...
        if (user) {
          expect(user.user_id).toEqual(me.user_id);
          return;
        }
      }

      throw new Error("Couldn't fetch user with the new session");
    }
  });
  ...
  it("add/list/remove OIDC identity", async () => {
    ...
    const resp = await apiClient.identityAdd({ oidc_token: oidcToken });
    if (resp.requiresMfa()) {
      const totpSecret = process.env["CS_USER_TOTP_SECRET"];
      expect(totpSecret).toBeDefined();
      await resp.totpApprove(client, authenticator.generate(totpSecret!));
    }
    try {
  ...
  it.each([true, false])(
    `create and delete third party user (using proof: %s)`,
    async (useProof: boolean) => {
      const oidcToken = await loadCognitoOidcToken();
      ...
      const newUserId = await org.createOidcUser(useProof ? proof : proof.identity!, email);
  ...
  it("OidcAuth appears in the audit log", async () => {
    const startTime = Math.floor(Date.now() / 1000) - 60;
    console.log("Exchanging token to produce an OidcAuth audit log event");
    const oidcToken = await loadCognitoOidcToken();

    // Authenticate using the OIDC token, which produces an `OidcAuth` event.
    await CubeSignerClient.createOidcSession(env, orgId, oidcToken, ["manage:readonly"]);

    // Audit log events propagate asynchronously. Retry for a while until the `OidcAuth` event
    // shows up.
    for (let i = 0; i < 30; i++) {
      const entries = await apiClient
        .orgQueryAuditLog({ start_time: startTime, events: ["OidcAuth"] })
        .fetchAll();
      const found = entries.find((e) => e.event === "OidcAuth");
      if (found !== undefined) {
        // `scopes` is stored as a comma-joined string in the DB but parsed back into an array.
        expect(Array.isArray(found.scopes)).toBe(true);
        expect(found.scopes.some((s) => s.startsWith("manage:") || s.startsWith("sign:"))).toBe(
          true,
        );
        return;
      }
      console.log(`OidcAuth event not found yet (attempt ${i + 1}), waiting`);
      await delay(2000);
    }

    expect.fail("Couldn't find OidcAuth event in audit log after waiting");
  });
});
