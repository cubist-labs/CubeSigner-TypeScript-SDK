...
describe("OidcSessionManager", () => {
  ...
  it("add/list/remove OIDC identity", async () => {
    ...
    try {
      ...
      console.log(resp);
      expect(resp.identities).to.deep.contain(oidcIdentity);
      await apiClient.identityRemove(oidcIdentity);
      resp = await apiClient.identityList();
      console.log(resp);
      expect(resp.identities).to.not.deep.contain(oidcIdentity);
    } catch (e) {
  ...
  it("list orgs", async () => {
    ...
    expect(orgs.map((o) => o.org_id)).to.contain(orgId);
  });
