...
export async function deleteKeys(keys: Key[]) {
  ...
  for (let i = 0; i < keys.length; i += chunkSize) {
    ...
    await Promise.all(chunk.map((k) => k.delete())).catch((e) => {
      if ((e as ErrResponse).errorCode !== "KeyNotFound") {
        console.log("WARNING: could not delete some keys", e);
      }
    });
  }
}
...
export async function deleteRoles(roles: Role[]) {
  ...
  for (const role of roles) {
    ...
    try {
      ...
      if ((e as ErrResponse).errorCode !== "RoleNotFound") {
...
/**
 * Delete the given C2F functions.
 * This will fail if any function is still attached to a key, role or key-in-role.
 *
 * @param functions functions to delete.
 */
export async function deleteC2FFunctions(functions: C2FFunction[]) {
  await deletePolicies(functions);
}
