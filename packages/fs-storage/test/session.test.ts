...
describe("Session Management", () => {
  ...
  it("switch regions", async () => {
    const root = client.env.SignerApiRoot;
    const c = client.withPreferredEnv(multiRegionEnvs.gamma.regional("ap-northeast-1"));
    expect(client.env.SignerApiRoot).to.eq(root);
    expect(c).to.not.eq(client);
    expect(c.env.SignerApiRoot).to.eq("https://ap-northeast-1.gamma.signer.cubist.dev");
  });
