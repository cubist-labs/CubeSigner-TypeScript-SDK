...
describe("Session Management", () => {
  ...
  it("rejects non-permitted session region", async () => {
    const root = client.env.SignerApiRoot;
    if (!root.endsWith("gamma.signer.cubist.dev")) {
      expect(() =>
        client.withPreferredEnv(multiRegionEnvs.gamma.regional("ap-northeast-1")),
      ).to.throw(
        "The current session does not allow the 'https://ap-northeast-1.gamma.signer.cubist.dev' environment",
      );
      expect(client.env.SignerApiRoot).to.eq(root);
    }
  });
  ...
  it("isStale", async () => {
    ...
    vi.useFakeTimers();
    vi.setSystemTime(new Date().getTime() + 65 * 1000);
