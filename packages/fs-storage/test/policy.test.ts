...
describe("Named Policy", () => {
  ...
  it("bucket acl", async () => {
    const acl: BucketAce[] = [
      { subjects: "*", actions: ["read:key:value"] },
      { subjects: ["User#123"], actions: "*", bucket_keys: ["key1"] },
    ];
    const bucketName = "unused bucket";
    await client.apiClient.bucketUpdate(bucketName, { acl });
    const bucket = await client.apiClient.bucketGet(bucketName);
    expect(bucket.acl).to.deep.equal(acl);
    const buckets = await client.apiClient.bucketsList({ size: 10, all: true }).fetch();
    expect(buckets).to.deep.include(bucket);

    const metadata = `Set at: ${Date.now()}`;
    const bucket2 = await client.apiClient
      .bucketUpdate(bucketName, {
        acl: null,
        metadata,
      })
      .then((b) => b.data());
    expect(bucket2.metadata).to.equal(metadata);
  });
