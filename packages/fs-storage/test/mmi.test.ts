 ...
 */
async function createAndVerifyCustomerProof(cs: CubeSignerClient) {
  const jwt = await cs.apiClient.getCustomerProof();
...
describe("Mmi/CubeSignerClient", () => {
  ...
  it("verify user customer proof", async () => {
    // verify the customer proof of the current user session
    const result = await createAndVerifyCustomerProof(client);
    expect(result.payload.mmi?.customerId).to.eql(resolvedOrgId);
  ...
  it("fail to obtain customer proof without the mmi:* scope", async () => {
    ...
    const err = await assertErrorCode(
      ...
      createAndVerifyCustomerProof(rsClient),
    );
    expect(err instanceof ErrResponse).to.be.true;
  });

  it("verify role customer proof", async () => {
    // create a role
    const role = await client.org().createRole();
    createdRoles.push(role);
    ...
    const result = await createAndVerifyCustomerProof(rsClient);
    expect(result.payload.mmi?.customerId).to.eql(resolvedOrgId);
