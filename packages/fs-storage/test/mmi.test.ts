...
/**
 * Obtain then verify a CubeSigner session identity proof.
 *
 * @param cs CubeSigner client
 * @param aud Intended audience
 * @returns The JWT verification result
 */
async function createAndVerifyCustomerProof(cs: CubeSignerClient, aud: string) {
  const jwt = await cs.apiClient.getCustomerProof(aud);
  const jwksUrl = cs.apiClient.customerProofJwksUrl();
...
describe("Mmi/CubeSignerClient", () => {
  ...
  beforeAll(async () => {
    ...
    org = client.org();
    resolvedOrgId = await org.fetch().then((o) => o.org_id);

    createdKeys.push(
      await org.createKey(Secp256k1.Evm, undefined, {
        metadata: "MMI Test Key #1",
        policy: [AllowEip191Signing, AllowEip712Signing],
      }),
    );
    createdKeys.push(
      await org.createKey(Secp256k1.Evm, undefined, {
  ...
  afterAll(async () => {
    await Promise.all([deleteKeys(createdKeys), deleteRoles(createdRoles)]);
  ...
  for (const aud of ["mmi", "cube-pay"]) {
    it(`verify user customer proof for '${aud}'`, async () => {
      // verify the customer proof of the current user session
      const result = await createAndVerifyCustomerProof(client, aud);
      expect(result.payload.aud).to.eql(aud);
      expect(aud === "mmi" ? result.payload.mmi?.customerId : result.payload.org_id).to.eql(
        resolvedOrgId,
      );
      const myUserId = await client.user().then((u) => u.user_id);
      expect(result.payload.sub).to.eql(myUserId);
    });
  }

  it("fail to obtain customer proof without the mmi:* scope", async () => {
    // create a role
    const role = await client.org().createRole();
    createdRoles.push(role);

    // create a role session and a client
    const rs = await role.createSession("test-no-mmi");
    const rsClient = await CubeSignerClient.create(rs);

    // assert that obtaining a proof fails with improper scope
    const err = await assertErrorCode(
      "ImproperSessionScope",
      createAndVerifyCustomerProof(rsClient, "mmi"),
    );
    expect(err instanceof ErrResponse).to.be.true;
  });

  for (const aud of ["mmi", "cube-pay"]) {
    it(`verify role customer proof for '${aud}'`, async () => {
      // create a role
      const role = await client.org().createRole();
      createdRoles.push(role);

      // create a role session and a client
      const rs = await role.createSession("test-mmi", undefined, ["mmi:*"]);
      const rsClient = await CubeSignerClient.create(rs);

      // verify its customer proof
      const result = await createAndVerifyCustomerProof(rsClient, aud);
      expect(result.payload.aud).to.eql(aud);
      expect(aud === "mmi" ? result.payload.mmi?.customerId : result.payload.org_id).to.eql(
        resolvedOrgId,
      );
      expect(result.payload.sub).to.eql(role.id);
    });
  }
  ...
  const mkTransaction = async () => {
    const messageId = (await mmiJsonRpcCall(client, "custodian_createTransaction", [
      {
        from: createdKeys[0].materialId,
        to: createdKeys[1].materialId,
        type: "0x2",
        value: "0x1",
        gas: "0x5208",
  ...
  const mkTypedData = async () => {
    const messageId = (await mmiJsonRpcCall(client, "custodian_signTypedData", [
      {
        address: createdKeys[0].materialId,
        data: {
