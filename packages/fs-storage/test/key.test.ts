...
describe("Key", () => {
  ...
  it("attest", async () => {
    const policy = ["AllowEip191Signing"];
    const editPolicy: EditPolicy = {
      applies_to_scopes: { AllOf: ["manage:key:update:*"] },
    ...
    createdKeys.push(key);
    const jwt = await key.attest().then((r) => r.jwt);

    // verify JWT with jose
    const jwksUrl = `${client.env.SignerApiRoot.replace(/\/$/, "")}/v0/attestation/.well-known/jwks.json`;
    const jwks = createRemoteJWKSet(new URL(jwksUrl), {
      headers: { "User-Agent": "node" },
    });
    const result = await jwtVerify(jwt, jwks);
    const keyInfo = result.payload.key_info as KeyInfo;
