...
describe("Key", () => {
  ...
  it("attest", async () => {
    const policy: KeyPolicyRule[] = ["AllowEip191Signing"];
    const editPolicy: EditPolicy = {
      applies_to_scopes: { AllOf: ["manage:key:update:*"] satisfies ExplicitScope[] },
    ...
    const attestAndVerify = (query?: Parameters<Key["attest"]>["0"]) =>
      key.attest(query).then((r) => jwtVerify<KeyAttestationClaims>(r.jwt, jwks));

    // get attestation
    {
      const result = await attestAndVerify();
      const keyInfo = result.payload.key_info;
      expect(keyInfo.key_id).to.eql(key.id);
      expect(keyInfo.owner).to.eql(me);
      expect(keyInfo.policy).to.eql(policy);
      expect(keyInfo.edit_policy).to.eql(editPolicy);
      expect(result.payload.key_roles).to.be.oneOf([null, undefined]);
    }

    // get attestation with roles
    {
      const result = await attestAndVerify({ include_roles: true });
      const keyInfo = result.payload.key_info;
      expect(keyInfo.key_id).to.eql(key.id);
      expect(keyInfo.owner).to.eql(me);
      expect(keyInfo.policy).to.eql(policy);
      expect(keyInfo.edit_policy).to.eql(editPolicy);
      expect(result.payload.key_roles).to.eql([]);
    }

    // create a role and the key to it
    const role = await org.createRole();
    createdRoles.push(role);
    const keyInRolePolicy: KeyPolicy = ["AllowDiffieHellmanExchange"];
    ...
    // get attestation with roles
    {
      ...
      expect(keyInfo.policy).to.eql(policy);
      expect(keyInfo.edit_policy).to.eql(editPolicy);
      expect(result.payload.key_roles?.length).to.eql(1);
      const key_role = result.payload.key_roles?.[0];
      expect(key_role?.key_id).to.eql(key.id);
      expect(key_role?.role_id).to.eql(role.id);
      expect(key_role?.policy).to.eql(keyInRolePolicy);
      expect(key_role?.key_info).to.be.undefined;
    }
  });
  ...
  it("if then else", async () => {
    ...
    const msgB64 = "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA=";
    const denyMessage = "Everything else is denied";

    const signEvm = () =>
      client.apiClient.signEvm(key, {
        chain_id: 1,
        tx: { type: "0x00", value: "0x1000", to: key.materialId },
      });
    const signEip191 = (data: string) => client.apiClient.signEip191(key, { data });
    const signBlob = (msg: string) => client.apiClient.signBlob(key, { message_base64: msg });
    ...
    await key.setPolicy([
      ...
      { Not: "Deny" },
      {
        IfThenElse: {
          conditionals: [
            {
              if: { OperationAllowlist: ["BlobSign"] },
              then: { Cel: `body.message_base64 == '${msgB64}'` },
            },
            {
              if: { OperationAllowlist: ["Eip191Sign"] },
              then: { Cel: "body.data.startsWith('0x1234')" },
            },
          ],
          else: {
            outcome: "Deny",
    ...
    // exercise allowed BlobSign
    await signBlob(msgB64).then((resp) => resp.data());

    // exercise denied BlobSign
    await assertErrorCode(
      "CelPolicyDenied",
      signBlob("41X4bQ2fzp0I703VNTSA47sMmICcZXmHas6RuQDvAqk="),
    );

    // exercise allowed EIP-191
    await signEip191("0x12345678").then((resp) => resp.data());

    // exercise denied EIP-191
    await assertErrorCode("CelPolicyDenied", signEip191("0x12"));

    // exercise denied Eth1Sign
    const err = await assertErrorCode("ExplicitlyDenied", signEvm());
    expect(err.message).to.contain(denyMessage);
  });
