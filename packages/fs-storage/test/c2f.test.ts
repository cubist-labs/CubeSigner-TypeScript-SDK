...
describe("Policy Secrets", () => {
  ...
  beforeAll(async () => {
    ...
    secretName = `test_secret_${randomInt(10000000)}`;
  });

  afterAll(async () => {
    const { secrets } = await client.apiClient.policySecretsGet();
    if (secrets.some((s) => s.name === secretName)) {
      await client.apiClient.policySecretDelete(secretName).then((r) => r.data());
    }
  });

  it("set, list, and delete a policy secret", async () => {
    console.log(`Set secret ${secretName}`);
    await client.apiClient
      .policySecretSet(secretName, { value: "test_value" })
      .then((r) => r.data());
    ...
    console.log(`Delete secret ${secretName}`);
    await client.apiClient.policySecretDelete(secretName).then((r) => r.data());
