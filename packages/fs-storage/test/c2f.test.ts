...
describe("Confidential Cloud Functions (C2F)", () => {
  ...
  it("captures C2F function output", async () => {
    ...
    expect(printResp.stderrBytes).to.be.empty;
    expect(new TextDecoder("utf-8").decode(printResp.stdoutBytes)).to.eql(
    ...
    expect(allowResp.stderrBytes).to.be.empty;
    expect(allowResp.stdoutBytes).to.be.empty;
  });
...
describe("Policy Secrets", () => {
  ...
  const secretName = `test_secret_${randomInt(10000000)}`;

  beforeAll(async () => {
    client = await newCubeSigner();
  });

  it("set, list, delete a policy secret", async () => {
    const respSet = await client.apiClient.policySecretSet(secretName, { value: "test_value" });
    expect(respSet.isSuccess()).to.be.true;

    console.log("List secrets and assert the secret is present");
    const { secrets: secretsAfterSet } = await client.apiClient.policySecretsGet();
    expect(secretsAfterSet.map((s) => s.name)).to.include(secretName);

    const respDel = await client.apiClient.policySecretDelete(secretName);
    expect(respDel.isSuccess()).to.be.true;

    console.log("List secrets and assert the secret is gone");
    const { secrets: secretsAfterDelete } = await client.apiClient.policySecretsGet();
    expect(secretsAfterDelete.map((s) => s.name)).not.to.include(secretName);
  });
