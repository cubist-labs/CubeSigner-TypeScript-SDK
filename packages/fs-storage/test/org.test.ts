...
import type { MfaType, QueryMetricsRequest, UserInOrgInfo } from "@cubist-labs/cubesigner-sdk";
import {
  Org,
  type OrgPolicy,
  type OrgEvents,
  ...
  it("set disallowed allowed mfa types", async () => {
    await Promise.all([
      ...
      assertErrorCode(
        "InvalidUpdateOrgRequest",
        org.setAllowedMfaTypes({
          Default: ["FidoKey#bExvAss9JCdBwR5PUd3Rn2tqHuU=" as MfaType],
        }),
      ),
    ]);
  });
