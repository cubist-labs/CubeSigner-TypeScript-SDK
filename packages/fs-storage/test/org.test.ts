...
import type { QueryMetricsRequest, RolePolicy, UserInOrgInfo } from "@cubist-labs/cubesigner-sdk";
import {
  Org,
  type OrgPolicy,
  type OrgEvents,
...
import { assert4xx, assertError, assertErrorCode, delay } from "./helpers";
...
describe("Org/CubeSignerClient", () => {
  ...
  it.skip("create child org", async () => {
    ...
    // create a role and a role session in the child org
    let role = undefined;
    try {
      role = await childOrg.createRole();
      const rs = await role.createSession("test purpose");
      expect(rs.org_id).to.eq(childOrg.id);
    } finally {
      await role?.delete();
    }
  });
  ...
  it("Update ext props", async () => {
    const props = await org.getExtendedProperties();
    try {
      await org.setAlienKeyCountThreshold(3);
      await org.setAliensCanUpdateKeyPolicy(true);
      const newProps = await org.getExtendedProperties();
      expect(newProps?.alien_key_count_threshold).to.eq(3);
      expect(newProps?.aliens_can_update_key_policy).to.eq(true);
    } finally {
      await org.setExtendedProperties({
        alien_key_count_threshold: props?.alien_key_count_threshold ?? 0,
        aliens_can_update_key_policy: props?.aliens_can_update_key_policy ?? false,
      });
    }
  });
  ...
  it("list users", async () => {
    ...
    if (me.email) {
      const usersFilteredByEmail = await org.users(me.email);

      const foundByEmail = usersFilteredByEmail.find((u) => u.id === me.user_id);
      expect(foundByEmail).to.exist;
    }

    if (me.name) {
      const usersFilteredByName = await org.users(me.name);
      const foundByName = usersFilteredByName.find((u) => u.id === me.user_id);
      expect(foundByName).to.exist;
    }
  });
  ...
  it("query audit log", async () => {
    ...
    // sign then reject MFA
    await key.signEvm({ chain_id: 1, tx: { type: "0x02" } }).then((r) => r.reject(client));

    // sign then approve MFA
    const resp = await key
      .signEvm({ chain_id: 1, tx: { type: "0x02" } })
      .then((r) => r.approve(client));
    ...
    for (const event of [
      ...
      let i = 0;
      while (true) {
        const entries = await org.queryAuditLog(startTime, { events: [event] });
        console.log(event, entries);
        totalEventsReceived += entries.length;
        if (entries.length > 0) break;
        expect(i).to.be.lessThan(10, `Exceeded retries for '${event}' audit event`);
        await delay(delayMs);
        delayMs *= 2;
        i += 1;
      }
    ...
    // query all events
    const entries = await org.queryAuditLog(startTime);
    expect(entries.length).to.be.greaterThanOrEqual(totalEventsReceived);
  });
  ...
  it("set-email-template", async () => {
    await assertErrorCode(
      ...
      client.apiClient.orgConfigureEmail("otp", {
        auth: { smtp: "smtps://user:password@smtp.example.com:465" },
        sender: "user@example.com",
        template: { body_template: "body", subject_template: "subject" },
      }),
    );
  });
  ...
  it("set sign policy", async () => {
    const originalPolicy = await org.signPolicy();
    try {
      const policy: RolePolicy = [];
      // start with an empty policy:
      await org.setSignPolicy(policy);
      expect(await org.signPolicy()).to.deep.equal(policy);
      // set policy:
      policy.push({ SourceIpAllowlist: ["0.0.0.0/0"] });
      await org.setSignPolicy(policy);
      expect(await org.signPolicy()).to.deep.equal(policy);
      // add to policy:
      policy.push({
        Or: [
          ...
          {
            SuiTxReceivers: ["0xc9837a0ad2d11468bbf847e3af4e3ede837bcc02a1be6faee621df1a8a403cbf"],
          },
          { Not: { OperationAllowlist: ["Eth1Sign", "SuiSign"] } },
        ],
      });
      await org.setSignPolicy(policy);
      expect(await org.signPolicy()).to.deep.equal(policy);
      ...
      // set to original policy
      await org.setSignPolicy(originalPolicy);
      expect(await org.signPolicy()).to.deep.equal(originalPolicy);
    }
  });

  it("create user (old parameter-style API)", async () => {
    const orgInfo = await client.apiClient.orgGet(org.id);
    if (orgInfo.default_invite_kind !== "Cognito") {
      // Getting a user by email (without accepting the invitation) only works with Cognito
      // We skip this test if the org is not using Cognito as the default invite kind
      return;
    }
    const email = "test+org+create+user+parameter+style@example.com";
    await org.createUser(email, "Test User", "Member", false);
    const user = await org.getUserByEmail(email);
    expect(user.users.at(0)?.email).to.equal(email);
    await org.deleteUser(user.users.at(0)!.id);
  });

  it("user info", async () => {
    const iss = "https://accounts.google.com";
    const sub = randomBytes(10).toString("hex");
    ...
    // retrieve user by oidc
    const byOidc = await org.getUserByOidc(iss, sub);
    expect(byOidc.user).to.not.be.null;
    const userByOidc = byOidc.user!; // guaranteed non-null
    expect(userId).to.equal(userByOidc.id);

    // delete it
    await org.deleteUser(user.id);

    // retrieve a user via email
    const email = "test+get+user+via+email@example.com";
    await org.createUser({
      name: "testGetUserViaEmail2",
      email,
      role: "Member",
      skip_email: true,
      invite_kind: "Cognito",
    ...
    const resp = await org.getUserByEmail(email);
    expect(resp.users.length).to.equal(1);
    const gotUserViaEmail = resp.users[0];
    expect(gotUserViaEmail.email).to.equal(email);

    await org.deleteUser(gotUserViaEmail.id);
  });
});
