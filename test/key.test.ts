...
import { CubeSigner, Ed25519, Org, Secp256k1, MnemonicType } from "../src";
...
describe("Key", () => {
  ...
  it("create mnemonic key, derive other key types", async () => {
    const key = await org.createKey(MnemonicType.Mnemonic);
    let keys = await org.keys(MnemonicType.Mnemonic);
    expect(keys.map((k) => k.id)).to.include(key.id);

    // test deriving eth key
    const ethKey = await org.deriveKey(Secp256k1.Evm, "m/44'/60'/0'/0/0", key.materialId);
    keys = await org.keys(Secp256k1.Evm);
    expect(keys.map((k) => k.id)).to.include(ethKey.id);

    // test deriving solana key
    const solanaKey = await org.deriveKey(Ed25519.Solana, "m/44'/60'/0'/0/0", key.materialId);
    keys = await org.keys(Ed25519.Solana);
    expect(keys.map((k) => k.id)).to.include(solanaKey.id);

    // test deriving btc key
    const btcKey = await org.deriveKey(Secp256k1.BtcTest, "m/44'/60'/0'/0/0", key.materialId);
