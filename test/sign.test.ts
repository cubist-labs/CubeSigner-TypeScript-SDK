...
describe("Sign", () => {
  ...
  beforeAll(async () => {
    ...
    const resp = await org.userResetTotpInit();
    if (resp.requiresMfa()) {
  ...
  it("addFido", async () => {
    // TODO: addFido requires user email; for all user's (like the one used by our CI validation,
    //       we don't have an email in the `User` DynamoDB record, so we rely on getting it from
    //       from Cognito and hence we need to use a Cognito session here)
    const cs = await CubeSignerClient.loadManagementSession();
