...
describe("Sign", () => {
  ...
  it("blob", async () => {
    expect(await role.enabled()).to.equal(true);
    for (const keyType of [Ed25519.Aptos, Ed25519.Cardano, Secp256k1.Ava]) {
      ...
      await assertPreconditionFailed(session.signBlob(edKey, req));

      {
        await role.addKey(edKey, ["AllowRawBlobSigning"]);
        const sig = await session.signBlob(edKey, req);
        console.log(`Signed blob: ${JSON.stringify(sig.data())}`);
      }

      console.log("Requiring MFA");
      await role.addKey(edKey, ["AllowRawBlobSigning", { RequireMfa: {} }]);

      // request then approve using CubeSigner management session
      {
        const resp = await session.signBlob(edKey, req);
        const sig = await resp.approve(cs);
