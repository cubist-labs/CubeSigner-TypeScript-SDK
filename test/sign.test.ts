...
describe("Sign", () => {
  ...
  beforeAll(async () => {
    ...
    const sessionMgr = await cs.oidcAuth(oidcToken, org.id, ["manage:mfa"]);
    userMfaSession = new SignerSession(sessionMgr);

    const resp = await cs.resetTotpStart();
    if (resp.requiresMfa()) {
      ...
      await cs.verifyTotp(authenticator.generate(totpSecret));
    }
  });
  ...
  it("evm", async () => {
    ...
    // do the same but disconnected from the original SignResponse object
    {
      const resp = await session.signEvm(secp, req);
      assert(resp.requiresMfa());
      const mfa = await cs.mfaApprove(org.id, resp.mfaId());
      assert(mfa.receipt);
      const sig = await session.signEvm(secp, req, {
        mfaId: mfa.id,
        mfaOrgId: org.id,
  ...
  it("blob", async () => {
    expect(await role.enabled()).to.equal(true);
    await Promise.all(
      [Ed25519.Aptos, Ed25519.Cardano, Secp256k1.Ava, Secp256k1.AvaTest, Stark].map(
        async (keyType) => {
