...
describe("Sign", () => {
  ...
  beforeAll(async () => {
    ...
    const sessionMgr = await cs.createOidcSession(oidcToken, org.id, ["sign:*"]);
    userSignerSession = new SignerSession(cs, sessionMgr, role.id);
  });
  ...
  it("eth2", async () => {
    ...
    const bls = await org.createKey(BLS.Eth2Deposited);
  ...
  it("blob", async () => {
    expect(await role.enabled()).to.equal(true);
    console.log("Creating a ED25519 key");
    const edAptos = await org.createKey(Ed25519.Aptos);

    console.log("Adding key to role");
    await role.addKey(edAptos);

    console.log("Signing a blob");
    const req = { message_base64: "L1kE9g59xD3fzYQQSR7340BwU9fGrP6EMfIFcyX/YBc=" };
    await assertPreconditionFailed(session.signBlob(edAptos, req));

    {
      await role.addKey(edAptos, ["AllowRawBlobSigning"]);
      const sig = await session.signBlob(edAptos, req);
      console.log(`Signed blob: ${JSON.stringify(sig.data())}`);
    }

    console.log("Requiring MFA");
    await role.addKey(edAptos, ["AllowRawBlobSigning", { RequireMfa: {} }]);

    // request then approve using CubeSigner management session
    {
      const resp = await session.signBlob(edAptos, req);
      const sig = await resp.approve();
  ...
  it("mfa", async () => {
    ...
    console.log("Signing a blob");
    const req = { message_base64: "L1kE9g59xD3fzYQQSR7340BwU9fGrP6EMfIFcyX/YBc=" };
    let resp = await session.signBlob(edAptos, req);
