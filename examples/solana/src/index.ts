...
async function main() {
  ...
  // get cubesigner key
  const from = (await signerSession.keys()).find((k) => k.material_id === fromPubkey.toBase58())!;
  // sign
  const sig = (await signerSession.signBlob(from.key_id, { message_base64: base64 })).data()
    .signature;
