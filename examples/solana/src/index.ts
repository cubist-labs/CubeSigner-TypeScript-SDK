...
async function main() {
  ...
  const resp = await client.apiClient.signSolana(FROM_ADDRESS, { message_base64: base64 });
  ...
  try {
    ...
    const blobSig = (await client.apiClient.signBlob(fromKeyId, { message_base64: base64 })).data()
      .signature;
