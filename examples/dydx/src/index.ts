...
import { StarkSignable, SignableOrder, asEcKeyPairPublic } from "@dydxprotocol/starkex-lib";
...
/**
 * Converts the public key of `KeyInfo` object to a `BasePoint`
 * @param {cs.KeyInfo} key CubeSigner key whose public key to convert
 * @return {curve.base.BasePoint} Corresponding base point
 */
function toBasePoint(key: cs.KeyInfo): curve.base.BasePoint {
 ...
 * @param {cs.KeyInfo} key Stark key to search for
 ...
 */
async function loadSignerSession(): Promise<cs.SignerSession> {
  const storage = CUBE_SIGNER_TOKEN
    ? new cs.MemorySessionStorage(JSON.parse(atob(CUBE_SIGNER_TOKEN)))
    : undefined;
  return await cs.CubeSigner.loadSignerSession(storage);
}
...
class Program {
  ...
  readonly cubesigner: cs.SignerSession;
  /** Stark key */
  readonly starkKey: cs.KeyInfo;
  /** Eth key */
  readonly ethKey: cs.KeyInfo;
  ...
  async ensureApiKey() {
    if (!this.dydx.apiKeyCredentials) {
      const createApiKeyTag = "dydx(crateApiKey)";
  ...
  //   async createAccount(): Promise<AccountResponseObject> {
  //     this.dydx.private.createAccount()
  //   }
  ...
  async ensureTopUp(): Promise<AccountResponseObject> {
    ...
    const balance = parseFloat(acc.quoteBalance);
    if (balance < 1000) {
  ...
  async starkSign<T>(signable: StarkSignable<T>): Promise<string> {
    ...
    const sig = await this.cubesigner.signBlob(this.starkKey.id, { message_base64 });
   ...
   * @param {cs.KeyInfo} starkKey CubeSigner stark key
   * @param {cs.KeyInfo} ethKey CubeSigner eth key
   */
  constructor(
    dydx: DydxClient,
...
`,
  ...
  cli
    ...
    .action(async () => {
      ...
      const sessionStarkKeys = (await prog.cubesigner.keys()).filter((k) => k.type === "Stark");
