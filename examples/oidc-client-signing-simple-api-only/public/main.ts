...
/** Given an oidc token:
 * 1. Exchange the OIDC token for a CubeSigner idenity proof
 * 2. If there is no actual user associated with the identiy, call our backend
 ...
 * @param {string} oidcToken The OIDC token to use for authentication. 
 */
 ...
 * @param {IdentityProof} proof The identity proof used to create the user.
 */
 ...
 * @param {NewSessionResponse} session The current session.
 * @return {Promise<string[]>} A promise that resolves to an array of EVM key addresses.
 */
 ...
 * @param {string} oidcToken The OIDC token to use for authentication.
 * @return {Promise<IdentityProof>} A promise that resolves to the identity proof.
 */
 ...
 * @param {string} oidcToken The OIDC token to use for authentication.
 * @param {string[]} scopes The scopes to be used for the session.
 * @return {Promise<NewSessionResponse>} A promise that resolves to the new session response.
 */
 ...
 * @param {NewSessionResponse} session The current session.
 * @return {Promise<NewSessionResponse>} A promise that resolves to a new session if expired and old one otherwise.
 */
 ...
 * @param {NewSessionResponse} session The current session.
 * @param {string} from The address from which to sign the transaction.
 * @param {Eth1SignRequest} req The signing request.
 * @return {Promise<Eth1SignResponse>} A promise that resolves to the signed transaction.
 */
