...
import type {
  ...
  EnvInterface,
  ErrResponse,
  IdentityProof} from "@cubist-labs/cubesigner-sdk";
import {
  CubeSignerClient,
  Secp256k1,
...
let orgClient: CubeSignerClient | undefined = undefined;
 ...
 * @returns A URLSearchParams built from query parameters
 */
 ...
 */
async function assertError<T>(t: Promise<T>): Promise<ErrResponse> {
  let result: T;
  try {
    result = await t;
  } catch (e) {
 ...
 */
async function assertErrorCode<T>(errCode: CsErrCode, t: Promise<T>): Promise<ErrResponse> {
  const err = await assertError(t);
  assert(err.errorCode === errCode, `Expected error code ${errCode}, got ${err.errorCode}`);
  return err;
}
 ...
 *
 * @returns CubeSigner for an org owner.
 */
async function getClient(): Promise<CubeSignerClient> {
  if (!orgClient) {
    orgClient = await CubeSignerClient.create(defaultManagementSessionManager())
    env = orgClient.env;
  }
  return orgClient;
}
...
app.get(PAGE_MFA_INIT, async (req, res) => {
  ...
  const totpResp = await mfaClient.resetTotp();
  ...
  <html>
    ...
    <body>
      ...
      <script type="text/javascript">
        new QRious({element: document.getElementById("qrcode"), value: "${totp.url}"});
      </script>

      <form action="${PAGE_MFA_INIT_VERIFY_TOTP}">
        <input type="hidden" id="orgId" name="orgId" value="${orgId}"/>
        <input type="hidden" id="totpId" name="totpId" value="${totp.id}"/>
...
app.get(PAGE_MFA_INIT_VERIFY_TOTP, async (req, res) => {
  ...
  await mfaClient.getTotpChallenge(totpId).answer(totpCode);
  res.redirect(`${PAGE_CUBE_SIGNER_LOGIN}?${p({ orgId })}`);
});
...
app.get(PAGE_CUBE_SIGNER_LOGIN, async (req, res) => {
  ...
  const resp = await CubeSignerClient.createOidcSession(env, orgId, oidcToken, ["sign:*", "manage:*"], lifetimes, mfa);

  // MFA required -> go to "Approve MFA"
  const newMfaId = resp.mfaId();
  if (newMfaId !== undefined) {
    const tmpClient = await resp.mfaClient()!;
    assert(tmpClient);

    mfaClient = tmpClient;

    // sanity check that this session does not give access to normal management endpoints
    await assertErrorCode("ImproperSessionScope", tmpClient.org().fetch());
    await assertErrorCode("SessionWithoutAnyScopeUnder", tmpClient.sessionKeys());

    if (newlyCreatedUserId) {
      res.redirect(`${PAGE_MFA_INIT}?${p({ orgId, userId: newlyCreatedUserId })}`);
    } else {
      res.redirect(`${PAGE_MFA_APPROVE}?${p({ orgId, mfaId: newMfaId })}`);
    }
    return;
  }
  ...
  thirdPartyUserCubeSigner = await CubeSignerClient.create(sessionData);

  // obtain authentication/identity proof
  const identityProof = await CubeSignerClient.proveOidcIdentity(env, orgId, oidcToken);
...
app.get(PAGE_MFA_APPROVE_TOTP, async (req, res) => {
  ...
  try {
    const status = await mfaClient.org().getMfaRequest(mfaId).totpApprove(totpCode);
    const receipt = await status.receipt();
    if (receipt) {
      // redirect to login if approved
      res.redirect(`${PAGE_CUBE_SIGNER_LOGIN}?${p({ orgId, mfaId, mfaConf: receipt.mfaConf })}`);
      return;
    } else {
...
app.get(PAGE_HOME, async (req, res) => {
  ...
  if (!thirdPartyUserCubeSigner || !thirdPartyUserCubeSigner) {
    res.redirect("/");
    return;
  }

  const userInfo = await thirdPartyUserCubeSigner.user();
  const key = (await thirdPartyUserCubeSigner.sessionKeys())[0];
