...
/**
 * @param f Function to call and assert that it throws.
 */
...
app.get(PAGE_CUBE_SIGNER_LOGIN, async (req, res) => {
  ...
  if (newMfaId !== undefined) {
    ...
    await assertThrowsRequiredScope(() => tmpClient.org().fetch());
    await assertThrowsRequiredScope(() => tmpClient.sessionKeys());
