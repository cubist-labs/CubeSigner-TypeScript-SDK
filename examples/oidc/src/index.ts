...
let orgOwnerCubeSigner: CubeSigner | undefined = undefined;
let thirdPartyUserCubeSigner: CubeSigner | undefined = undefined;
...
async function assertThrowsRequiredScope<T>(f: () => Promise<T>) {
  ...
  if (!caught) {
    ...
    assert(`${caught}`.includes("Session does not have required scope"));
  }
}
 ...
 *
 * @return {Promise<CubeSigner>} CubeSigner for an org owner.
 */
async function getCubeSigner(): Promise<CubeSigner> {
  if (!orgOwnerCubeSigner) {
    orgOwnerCubeSigner = await CubeSigner.loadManagementSession();
...
app.post("/oauth2callback", async (req, res) => {
  ...
  const user = await cubesigner.aboutMe();
  ...
  const org = await cubesigner.getOrg(orgId);
...
app.get(PAGE_DELETE_OIDC_USER, async (req, res) => {
  ...
  await org.oidcVerify(identity);

  console.log("Deleting", identity);
  await org.deleteOidcUser(identity);
...
app.get(PAGE_MFA_INIT, async (req, res) => {
  ...
  const oidcCs = new CubeSigner({ sessionMgr: mfaSessionMgr });
  const totp = await oidcCs.initTotp();

  const nextUrl = `${PAGE_CUBE_SIGNER_LOGIN}?${p({ orgId })}`;
  res.status(200).send(`
  <html>
    <head>
    ...
    <body>
      ...
      <script type="text/javascript">
        new QRious({element: document.getElementById("qrcode"), value: "${totp.totp_url}"});
      </script>

      <p><a href="${nextUrl}">Continue to Log In</a>
    </body>
  </html>
  `);
});
...
app.get(PAGE_CUBE_SIGNER_LOGIN, async (req, res) => {
  ...
  const mfa = mfaId && mfaConf ? { mfaId, mfaConf } : undefined;
  ...
  const resp = await cs.oidcLogin(oidcToken, orgId, ["sign:*", "manage:*"], lifetimes, mfa);
  ...
  if (resp.requiresMfa()) {
    ...
    const tmpCs = new CubeSigner({ sessionMgr: mfaSessionMgr });
    assertThrowsRequiredScope(() => tmpCs.aboutMe());
    assertThrowsRequiredScope(() => tmpCs.resetTotp());
    assertThrowsRequiredScope(() => tmpCs.getOrg(orgId));
  ...
  thirdPartyUserCubeSigner = new CubeSigner({ sessionMgr });
  thirdPartyUserSignerSession = new SignerSession(sessionMgr);

  // obtain authentication/identity proof
  const identityProof = await cs.oidcProve(oidcToken, orgId);
...
app.get(PAGE_MFA_APPROVE, async (req, res) => {
  ...
  const oidcCs = new CubeSigner({ sessionMgr: mfaSessionMgr });
  const mfaInfo = await oidcCs.mfaGet(orgId, mfaId);
