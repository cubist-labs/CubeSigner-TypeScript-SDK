...
/** @returns The root react component for our app */
function App() {
  ...
  const logoutCb = useCallback(() => manager.setSession(), [manager]);
...
/** @returns A super minimal wallet UI that allows selecting a key and signing a test transaction */
function Wallet() {
  ...
  useEffect(() => {
    void client?.sessionKeys().then(setKeys);
  }, [client]);
 ...
 *
 * @returns A client and a manager persisting the session in local storage
 */
function useClient() {
