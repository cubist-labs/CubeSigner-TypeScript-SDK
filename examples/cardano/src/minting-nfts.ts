 ...
 * @param {KeyInfo} accountKey The account key to mint from.
...
async function mintNft(
  ...
  const _accountKeyHash = keyHash(accountKey);
  ...
  const witness0 = await makeWitness(signerSession, policy.keyInfo, txHash);
  const witness1 = await makeWitness(signerSession, accountKey, txHash);
