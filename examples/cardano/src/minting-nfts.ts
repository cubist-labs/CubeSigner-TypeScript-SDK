...
async function mintNft(
  ...
  txBuilder.add_key_input(
    ...
    CardanoWasm.TransactionInput.new(
      CardanoWasm.TransactionHash.from_bytes(new Uint8Array(Buffer.from(utxo.tx_hash, "hex"))),
      utxo.tx_index,
    ),
  ...
  txBuilder.add_mint_asset_and_output_min_required_coin(
    mintScript,
    CardanoWasm.AssetName.new(new Uint8Array(Buffer.from(assetName))),
...
function pubKeyToBuf(pubKey: string): Buffer {
  ...
  return Buffer.from(blake2bHex(new Uint8Array(buf), undefined, 28), "hex");
}
...
function keyHash(publicKey: string): CardanoWasm.Ed25519KeyHash {
  return CardanoWasm.Ed25519KeyHash.from_bytes(new Uint8Array(pubKeyToBuf(publicKey)));
}
