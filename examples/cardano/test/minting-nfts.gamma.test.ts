...
async function mintNft(
  ...
  const _accountKeyHash = keyHash(accountKey.key);
  ...
  const _policyKeyHash = keyHash(policy.key);
  ...
  const csPolicyWitness: MkVkeywitness = async (txHash) => {
    return await makeWitness(policy.key, txHash);
  };
  const csAccountWitness: MkVkeywitness = async (txHash) => {
    return await makeWitness(accountKey.key, txHash);
  };
...
describe("minting an NFT using CubeSigner", () => {
  it("should produce same signature as local", async () => {
    ...
    // The token is passed via env variable, decode and parse it,
    const signerSession = await cs.CubeSignerClient.create(CUBE_SIGNER_TOKEN);
 ...
 * @returns key hash
 */
function keyHash(key: Key): CardanoWasm.Ed25519KeyHash {
  return CardanoWasm.Ed25519KeyHash.from_bytes(pubKeyToBuf(key.publicKey));
}
