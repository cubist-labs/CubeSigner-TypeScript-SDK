...
class TransferUtil {
   ...
   * Constructor.
   *
   * @param client The CubeSigner client to use for signing
   * @param context The Avalanche context
   */
   ...
   *
   * @param cAddr C-chain address to transfer from (in Ethereum-style address format)
   * @param pAddr P-chain address to transfer to (in bech32 format)
   * @param nAvax Amount to transfer in nAVAX
   * @returns The transaction ids of the export and import transactions
   */
  async transferAvaxCtoP(cAddr: string, pAddr: string, nAvax: bigint): Promise<TransferTxIds> {
    ...
    const feeState = await this.pvmapi.getFeeState();
    const importTx = ava.pvm.newImportTx({
        utxos,
        feeState,
        sourceChainId: this.context.cBlockchainID,
        fromAddressesBytes: [ava.utils.bech32ToBytes(pAddr)],
        toAddressesBytes: [ava.utils.bech32ToBytes(pAddr)]
      },
      this.context
    ) 
  ...
  async transferAvaxPtoC(pAddr: string, cAddr: string, nAvax: bigint): Promise<TransferTxIds> {
    ...
    const exportTx = ava.pvm.newExportTx({
      utxos,
      feeState,
      destinationChainId: this.context.cBlockchainID,
      fromAddressesBytes: [ava.utils.bech32ToBytes(pAddr)],
      outputs: [ava.TransferableOutput.fromNative(this.context.avaxAssetID, nAvax, [cAddrBytes])],
    },
      this.context
    );
    const serExportTx = ava.utils.bufferToHex(exportTx.toBytes());
...
async function main() {
  ...
  console.log(
    `Transfered ${AMOUNT_P_TO_C} to ${C_CHAIN_ADDRESS} with txIDs: ${txIdsPToC.exportTxId} and ${txIdsPToC.importTxId}`,
  );
}
