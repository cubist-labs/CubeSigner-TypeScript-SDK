...
async function main() {
  ...
  const deposit: cs.BabylonStakingRequest = {
      ...
      finality_provider_pks: [FINALITY_PROVIDER_PK],
      lock_time: 64000,
      action: "deposit",
  };

  console.log("Creating deposit");
  const resp_dep = (await stakerKey.signBabylonStakingTxn(deposit)).data();
  ...
  const resp_eub = (await stakerKey.signBabylonStakingTxn(early_unbond)).data();
  ...
  const resp_wtl = (await stakerKey.signBabylonStakingTxn(withdraw_timelock)).data();
  ...
  const resp_wub = (await stakerKey.signBabylonStakingTxn(early_unbond_timelock)).data();
