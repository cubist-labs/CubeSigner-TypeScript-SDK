...
const AMOUNT: string = env("AMOUNT", "350.1234567")!;
...
async function main() {
  ...
  await signTransaction(transaction, signerSession, FROM_ADDRESS);
 ...
 * @param {StellarSdk.Transaction} transaction The transaction to sign.
 * @param {cs.SignerSession} signerSession The signer session.
 * @param {string} fromAddress The address of the account that is signing the transaction.
 */
async function signTransaction(
  ...
  // Sign transaction
  const hexSig = (
    await signerSession.signBlob(`Key#Stellar_${fromAddress}`, {
