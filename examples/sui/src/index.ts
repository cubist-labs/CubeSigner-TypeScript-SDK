...
async function main() {
  ...
  if (SUI_NETWORK !== "mainnet") {
    ...
    if (parseFloat(bal.totalBalance) < 1000000) {
      ...
      const resp = await suiFaucet.requestSuiFromFaucetV2({
        // use getFaucetHost to make sure you're using correct faucet address
        // you can also just use the address (see Sui TypeScript SDK Quick Start for values)
        host: suiFaucet.getFaucetHost(SUI_NETWORK),
        recipient: FROM_ADDRESS,
      });
      if (resp.coins_sent === null || resp.status !== "Success") {
        console.warn("Request SUI from faucet status", resp.status);
        throw new Error("Failed to top up");
      }
      console.log(resp.coins_sent);
      // wait for confirmation
      await Promise.all(resp.coins_sent.map(x => suiClient.waitForTransaction({ digest: x.transferTxDigest })));
    }
  }
