...
import { Utxo, estimateFee, getFeeRate, getUtxos, addrNetwork, BtcSigner } from "./util";
...
async function main() {
  ...
  const fromKey = (await signerSession.keys()).find((key) => key.materialId === FROM_ADDRESS);
  if (!fromKey) {
  ...
  await Promise.all(
    psbt.txInputs.map((_, i) => {
      const signer = new BtcSigner(fromKey, psbt, i, witnesses, network, signerSession);
